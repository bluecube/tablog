...
def typed_values(draw, unsigned_only=False) -> SearchStrategy[tuple[int_type.IntType, int]]:
...
@hypothesis.strategies.composite
def typed_lists(draw, min_size=0, max_size=None, unsigned_only=False) -> SearchStrategy[tuple[int_type.IntType, list[int]]]:
    """ Returns a hypothesis strategy that returns a tuple of a type and a list
    of values matching the type. """
    int_type = draw(int_types(unsigned_only=unsigned_only))
    lst = draw(hypothesis.strategies.lists(
        int_type_values(int_type),
        min_size=min_size,
        max_size=max_size
    ))
